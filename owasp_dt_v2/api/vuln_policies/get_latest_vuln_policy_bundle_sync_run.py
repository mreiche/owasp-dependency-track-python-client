from http import HTTPStatus
from typing import Any
from urllib.parse import quote
from uuid import UUID

import httpx

from ...client import AuthenticatedClient, Client
from ...models.problem_details import ProblemDetails
from ...models.vuln_policy_bundle_sync_status import VulnPolicyBundleSyncStatus
from ...types import Response


def _get_kwargs(
    uuid: UUID,
) -> dict[str, Any]:
    _kwargs: dict[str, Any] = {
        "method": "get",
        "url": "/vuln-policy-bundles/{uuid}/sync-runs/latest".format(
            uuid=quote(str(uuid), safe=""),
        ),
    }

    return _kwargs


def _parse_response(
    *, client: AuthenticatedClient | Client, response: httpx.Response
) -> ProblemDetails | VulnPolicyBundleSyncStatus:
    if response.status_code == 200:
        response_200 = VulnPolicyBundleSyncStatus.from_dict(response.json())

        return response_200

    if response.status_code == 401:
        response_401 = ProblemDetails.from_dict(response.json())

        return response_401

    if response.status_code == 403:
        response_403 = ProblemDetails.from_dict(response.json())

        return response_403

    if response.status_code == 404:
        response_404 = ProblemDetails.from_dict(response.json())

        return response_404

    response_default = ProblemDetails.from_dict(response.json())

    return response_default


def _build_response(
    *, client: AuthenticatedClient | Client, response: httpx.Response
) -> Response[ProblemDetails | VulnPolicyBundleSyncStatus]:
    return Response(
        status_code=HTTPStatus(response.status_code),
        content=response.content,
        headers=response.headers,
        parsed=_parse_response(client=client, response=response),
    )


def sync_detailed(
    uuid: UUID,
    *,
    client: AuthenticatedClient | Client,
) -> Response[ProblemDetails | VulnPolicyBundleSyncStatus]:
    """Get the latest vulnerability policy bundle sync run

     Returns the status of the most recent synchronization run for a given
    vulnerability policy bundle.

    Returns 404 if no sync run is available
    (e.g. none has been triggered yet, or the most recent run
    is no longer retained), or if the bundle is unknown.

    Requires permission `POLICY_MANAGEMENT` or `POLICY_MANAGEMENT_READ`.

    Args:
        uuid (UUID):

    Raises:
        errors.UnexpectedStatus: If the server returns an undocumented status code and Client.raise_on_unexpected_status is True.
        httpx.TimeoutException: If the request takes longer than Client.timeout.

    Returns:
        Response[ProblemDetails | VulnPolicyBundleSyncStatus]
    """

    kwargs = _get_kwargs(
        uuid=uuid,
    )

    response = client.get_httpx_client().request(
        **kwargs,
    )

    return _build_response(client=client, response=response)


def sync(
    uuid: UUID,
    *,
    client: AuthenticatedClient | Client,
) -> ProblemDetails | VulnPolicyBundleSyncStatus | None:
    """Get the latest vulnerability policy bundle sync run

     Returns the status of the most recent synchronization run for a given
    vulnerability policy bundle.

    Returns 404 if no sync run is available
    (e.g. none has been triggered yet, or the most recent run
    is no longer retained), or if the bundle is unknown.

    Requires permission `POLICY_MANAGEMENT` or `POLICY_MANAGEMENT_READ`.

    Args:
        uuid (UUID):

    Raises:
        errors.UnexpectedStatus: If the server returns an undocumented status code and Client.raise_on_unexpected_status is True.
        httpx.TimeoutException: If the request takes longer than Client.timeout.

    Returns:
        ProblemDetails | VulnPolicyBundleSyncStatus
    """

    return sync_detailed(
        uuid=uuid,
        client=client,
    ).parsed


async def asyncio_detailed(
    uuid: UUID,
    *,
    client: AuthenticatedClient | Client,
) -> Response[ProblemDetails | VulnPolicyBundleSyncStatus]:
    """Get the latest vulnerability policy bundle sync run

     Returns the status of the most recent synchronization run for a given
    vulnerability policy bundle.

    Returns 404 if no sync run is available
    (e.g. none has been triggered yet, or the most recent run
    is no longer retained), or if the bundle is unknown.

    Requires permission `POLICY_MANAGEMENT` or `POLICY_MANAGEMENT_READ`.

    Args:
        uuid (UUID):

    Raises:
        errors.UnexpectedStatus: If the server returns an undocumented status code and Client.raise_on_unexpected_status is True.
        httpx.TimeoutException: If the request takes longer than Client.timeout.

    Returns:
        Response[ProblemDetails | VulnPolicyBundleSyncStatus]
    """

    kwargs = _get_kwargs(
        uuid=uuid,
    )

    response = await client.get_async_httpx_client().request(**kwargs)

    return _build_response(client=client, response=response)


async def asyncio(
    uuid: UUID,
    *,
    client: AuthenticatedClient | Client,
) -> ProblemDetails | VulnPolicyBundleSyncStatus | None:
    """Get the latest vulnerability policy bundle sync run

     Returns the status of the most recent synchronization run for a given
    vulnerability policy bundle.

    Returns 404 if no sync run is available
    (e.g. none has been triggered yet, or the most recent run
    is no longer retained), or if the bundle is unknown.

    Requires permission `POLICY_MANAGEMENT` or `POLICY_MANAGEMENT_READ`.

    Args:
        uuid (UUID):

    Raises:
        errors.UnexpectedStatus: If the server returns an undocumented status code and Client.raise_on_unexpected_status is True.
        httpx.TimeoutException: If the request takes longer than Client.timeout.

    Returns:
        ProblemDetails | VulnPolicyBundleSyncStatus
    """

    return (
        await asyncio_detailed(
            uuid=uuid,
            client=client,
        )
    ).parsed
